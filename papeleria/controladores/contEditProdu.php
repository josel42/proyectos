...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $fechaModificacion = date("Y-m-d"); // Fecha actual
    ...
    $sql = "UPDATE productos SET nombreProducto = ?, descripcion = ?, precio = ?, 
            stock = ?, cantidad = ?, fechaModificacion = ?, categoria = ?, proveedor = ? WHERE idProducto = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssdsisssi", $nuevoNombre, $nuevaDescripcion, $nuevoPrecio, $nuevoStock, $nuevaCantidad, $fechaModificacion, $nuevaCategoria, $nuevoProveedor, $prodId);
